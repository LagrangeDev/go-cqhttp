...
func InitBase() {
	...
	if base.LittleWD != "" {
		if err := os.Chdir(base.LittleWD); err != nil {
			logrus.Fatalf("重置工作目录时出现错误: %v", err)
		}
	}
	base.Init()
}
...
func PrepareData() {
	rotateOptions := []rotatelogs.Option{
		...
		rotatelogs.WithMaxAge(base.LogAging),
	}
	if base.LogForceNew {
	...
	logrus.AddHook(global.NewLocalHook(w, consoleFormatter, fileFormatter, global.GetLogLevel(base.LogLevel)...))

	mkCacheDir := func(path string, _type string) {
		if !global.PathExists(path) {
			if err := os.MkdirAll(path, 0o755); err != nil {
				logrus.Fatalf("创建%s缓存文件夹失败: %v", _type, err)
			}
		}
	}
...
func LoginInteract() {
	...
	if (base.Account.Uin == 0 || (base.Account.Password == "" && !base.Account.Encrypt)) && !global.PathExists("session.token") {
		logrus.Warn("账号密码未配置, 将使用二维码登录.")
		if !base.FastStart {
			logrus.Warn("将在 5秒 后继续.")
			time.Sleep(time.Second * 5)
		}
	}

	logrus.Info("当前版本:", base.Version)
	if base.Debug {
		logrus.SetLevel(logrus.DebugLevel)
		logrus.Warnf("已开启Debug模式.")
	}
	if !global.FileExists("device.json") {
		logrus.Warn("虚拟设备信息不存在, 将自动生成随机设备.")
		device = auth.NewDeviceInfo(int(crypto.RandU32()))
		_ = device.Save("device.json")
		logrus.Info("已生成设备信息并保存到 device.json 文件.")
	} else {
		logrus.Info("将使用 device.json 内的设备信息运行Bot.")
		var err error
		if device, err = auth.LoadOrSaveDevice("device.json"); err != nil {
			logrus.Fatalf("加载设备信息失败: %v", err)
		}
	}
	...
	if base.Account.Encrypt {
		if !global.PathExists("password.encrypt") {
			if base.Account.Password == "" {
				logrus.Error("无法进行加密，请在配置文件中的添加密码后重新启动.")
			} else {
				logrus.Infof("密码加密已启用, 请输入Key对密码进行加密: (Enter 提交)")
				...
				logrus.Info("密码已加密，为了您的账号安全，请删除配置文件中的密码后重新启动.")
			}
			readLine()
			os.Exit(0)
		}
		if base.Account.Password != "" {
			logrus.Error("密码已加密，为了您的账号安全，请删除配置文件中的密码后重新启动.")
			readLine()
			os.Exit(0)
		}
		if len(byteKey) == 0 {
			logrus.Infof("密码加密已启用, 请输入Key对密码进行解密以继续: (Enter 提交)")
	...
	if !base.FastStart {
		logrus.Info("Bot将在5秒后登录并开始信息处理, 按 Ctrl+C 取消.")
		time.Sleep(time.Second * 5)
	}
	logrus.Info("开始尝试登录并同步消息...")
	app := auth.AppList["linux"]["3.2.10-25765"]
	logrus.Infof("使用协议: %s %s", app.OS, app.CurrentVersion)
	cli = newClient(app)
	...
	if global.FileExists("session.token") {
		...
		if err == nil {
			if base.Account.Uin != 0 && int64(sig.Uin) != base.Account.Uin {
				logrus.Warnf("警告: 配置文件内的QQ号 (%v) 与缓存内的QQ号 (%v) 不相同", base.Account.Uin, sig.Uin)
				logrus.Warnf("1. 使用会话缓存继续.")
				logrus.Warnf("2. 删除会话缓存并重启.")
				logrus.Warnf("请选择:")
				if readIfTTY("1") == "2" {
	...
	cli.DisconnectedEvent.Subscribe(func(_ *client.QQClient, e *client.DisconnectedEvent) {
		...
		logrus.Warnf("Bot已离线: %v", e.Message)
		time.Sleep(time.Second * time.Duration(base.Reconnect.Delay))
		for {
			if base.Reconnect.Disabled {
				logrus.Warnf("未启用自动重连, 将退出.")
				os.Exit(1)
			}
			if times > base.Reconnect.MaxTimes && base.Reconnect.MaxTimes != 0 {
				logrus.Fatalf("Bot重连次数超过限制, 停止")
			}
			times++
			if base.Reconnect.Interval > 0 {
				logrus.Warnf("将在 %v 秒后尝试重连. 重连次数：%v/%v", base.Reconnect.Interval, times, base.Reconnect.MaxTimes)
			...
			logrus.Warnf("快速重连失败: %v", err)
			if isQRCodeLogin {
				logrus.Fatalf("快速重连失败, 扫码登录无法恢复会话.")
			}
			logrus.Warnf("快速重连失败, 尝试普通登录. 这可能是因为其他端强行T下线导致的.")
			time.Sleep(time.Second)
			if err = qrcodeLogin(); err != nil {
				logrus.Errorf("登录时发生致命错误: %v", err)
			} else {
				saveToken()
				break
			}
	...
	logrus.Info("资源初始化完成, 开始处理信息.")
	logrus.Info("アトリは、高性能ですから!")
}
...
func newClient(app *auth.AppInfo) *client.QQClient {
	...
	//		logrus.Infof("收到服务器地址更新通知, 根据配置文件已忽略.")
	//		return false
	//	}
	//	logrus.Infof("收到服务器地址更新通知, 将在下一次重连时应用. ")
	//	return true
	//})
	if global.FileExists("address.txt") {
		logrus.Infof("检测到 address.txt 文件. 将覆盖目标IP.")
		addr := global.ReadAddrFile("address.txt")
		if len(addr) > 0 {
			c.SetCustomServer(addr)
		}
		logrus.Infof("读取到 %v 个自定义地址.", len(addr))
	}
...
func (p protocolLogger) Dump(data []byte, format string, arg ...any) {
	...
	logrus.Errorf("出现错误 %v. 详细信息已转储至文件 %v 请连同日志提交给开发者处理", message, dumpFile)
